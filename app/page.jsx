...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Preload all Wikipedia photos into browser cache while user is still
      // on the loading screen — cards will be instant from here on
      hardcoded.forEach((b, i) => {
        if (b.image) {
          const img = new Image();
          if (i < 3) img.fetchPriority = "high"; // first 3 load first
          img.src = b.image;
        }
      });
